...
The agent uses an 8-node directed graph:

- Loads MultiQC data from S3
- Builds FAISS vector index for semantic search
- Routes questions to specialized analysis nodes
- Finds relevant tables/plots in MultiQC reports
- Synthesizes answers using Claude Sonnet 4

**Look for**: Multi-step reasoning, autonomous decision-making, appropriate routing

### 2. Real-Time Streaming

Watch the progress indicators as the agent works through each step.

**Look for**: Transparency, smooth UX, no black-box feeling

### 3. Artifact Linking

The agent provides links to existing MultiQC tables and plots.

**Look for**: Relevant artifacts, correct presigned URLs, downloadable content

### 4. Natural Language Understanding

Ask questions in different ways—the agent should understand intent.

**Try**:

- "Which samples are bad?" vs "Show me quality issues"
- "Duplication rates" vs "How much duplication do we have?"

### 5. Conversation Context

The agent remembers previous messages in the conversation.

**Try**: Ask a follow-up question without repeating context

### 6. Interpretation & Recommendations

The agent doesn't just report data—it interprets results and suggests next steps.

**Look for**: Actionable insights, explanations of what metrics mean
...
- AWS ECS Fargate (auto-scales 1-4 tasks)
- RDS PostgreSQL (conversation persistence)
- CloudFront CDN (global distribution)
- S3 (MultiQC reports + artifacts)
...
**Not just RAG**: Traditional RAG retrieves documents and generates answers. This agent performs multi-step analysis:

1. Analyzes data structure
2. Calculates statistics and detects outliers
3. Retrieves relevant documentation
4. Finds appropriate visualizations
5. Synthesizes comprehensive answers

**Structured data advantage**: MultiQC outputs are highly consistent. The agent learns the schema once and applies it everywhere—no need to handle arbitrary formats.

**Production-ready**: Deployed on AWS with proper security, auto-scaling, zero-downtime updates, and monitoring.

## Known Limitations (MVP)

- **Single output type**: Currently only handles MultiQC reports
- **Basic intent routing**: Uses keyword matching (could be improved with LLM-based classification)
- **In-memory vector store**: FAISS index rebuilt per request (works fine for small datasets)
- **Limited knowledge base**: Only MultiQC documentation (could expand to nf-core docs, best practices)
...
**Questions?** Check the architecture diagrams in the repository or review the source code.

**Repository**: [GitHub URL provided separately]
